...
| **Producto 360** | Ficha con órbita de relaciones (grafo ego interactivo en canvas), relaciones agrupadas por tipo con justificación y confianza. Incluye el panel lateral **AI Sales Copilot**: diseño del panel (Fase 2, Paso 1), Context Builder (Fase 2, Paso 2) y tres habilidades funcionales servidas por un Local Response Provider sin IA ni red — **Explicar producto** (Paso 3), **Comparar productos** (Paso 4) y **Mejor alternativa** (Paso 5). |
...
│       ├── response-provider.js   # Puerto: registro del proveedor de respuestas activo del Copilot (explainProduct + compareProducts + bestAlternative)
│       ├── providers/
│       │   └── local-response-provider.js  # Proveedor local (sin IA, sin red) — "Explicar producto", "Comparar productos" y "Mejor alternativa"
